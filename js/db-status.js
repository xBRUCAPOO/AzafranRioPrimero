 ...
 *
 * Hace una petición liviana a la API (ahora el Worker de Cloudflare, ver
 * worker/src/index.js) para saber si está respondiendo:
 ...
 *
 * Depende de config.js (constante API_BASE), que debe cargarse ANTES que
 * este archivo en las 4 páginas.
 */

const DB_STATUS_URL = `${API_BASE}/api/clientes`;
...
async function verificarConexionDB() {
  ...
  try {
    ...
    // OJO: no alcanza con mirar res.ok: si la URL está mal configurada
    // (ver js/config.js) puede devolver 200 con HTML/texto en vez de JSON
    // real. Por eso hay que confirmar que la respuesta sea JSON válido,
    // igual que hace apiListar() en api-clientes.js.
    await res.json();
